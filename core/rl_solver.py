...
from utils import *
from bleu import evaluate, evaluate_captions
            ...
            for e in range(self.n_epochs):
                ...
                if self.print_bleu:
                    ...
                    for k in range(n_iters_val):
                        ...
                        captions_words_batch = np.array(val_captions[k * self.batch_size:(k + 1) * self.batch_size])

                        feed_dict = {self.model.features: features_batch, self.model.captions: captions_words_batch}
                        gen_cap = sess.run(greedy_caption, feed_dict=feed_dict)
                        all_gen_cap[k * self.batch_size:(k + 1) * self.batch_size] = gen_cap
                    masks, all_decoded = decode_captions_for_blue(all_gen_cap, self.model.idx_to_word)
                    for s in range(5):
    ...
    def test(self, data, split='train', attention_visualization=True, save_sampled_captions=False):
        ...
        with tf.Session(config=config) as sess:
            ...
            if self.print_bleu:
                ...
                save_pickle(all_decoded, "./data/train/train.candidate.captions.pkl")
                scores = evaluate(data_path='./data', split='train', get_scores=True)
