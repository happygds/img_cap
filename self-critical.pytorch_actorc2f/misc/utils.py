...
class RewardCriterion(nn.Module):
    ...
    def forward(self, input, seq, reward):
        ...
        mask = (seq > 0).float()
        mask = to_contiguous(torch.cat([mask.new(mask.size(0), 1).fill_(0), mask[:, :-1]], 1)).view(-1)
...
class CriticCriterion(nn.Module):
    def __init__(self):
        super(CriticCriterion, self).__init__()

    def forward(self, critmodel, sample_hid, seq, Q):
        V = critmodel(sample_hid)
        mask = (seq > 0).float()
        mask = Variable(to_contiguous(torch.cat([mask.new(mask.size(0), 1).fill_(0), mask[:, :-1]], 1)))
        output = (torch.pow(Q - V, 2) * mask).view(-1)
        #print('V shape:', V.data.cpu().numpy().shape)
        print('V:', V.data.cpu().numpy()[0][:5])
        print('Q:', Q.data.cpu().numpy()[0][:5])
        mask = mask.view(-1)
        output = torch.sum(output) / torch.sum(mask)
        return output, (Q - V).data.cpu().numpy()
