...
def get_actor_Q(data, gen_result, gamma, only_cider=True):
    batch_size = gen_result.size(0)  # batch_size = sample_size * seq_per_img
    seq_per_img = batch_size // len(data['gts'])
    gen_result = gen_result.cpu().numpy()
    mask = (gen_result) > 0
    res_discount = np.zeros((batch_size,), dtype='float32')
    res = OrderedDict()
    for i in range(batch_size):
    ...
    # print('len of res: ', len(res.keys()))
    # print('len of gts: ', len(gts.keys()))
    # print(res)
    # print('----------------------')
    # print(gts)
    ...
    if only_cider:
        ...
        _ = scores.mean()
    # print('Metric scores:', _)
    #print('seq:', gen_result[0])
    #print('mask', mask)
    Q = np.repeat(scores[:, np.newaxis], gen_result.shape[1], 1)
    for i in xrange(Q.shape[0]):
        for j in xrange(Q.shape[1] - 1, -1, -1):
            if not mask[i][j] or j == Q.shape[1] - 1:
                Q[i][j] = scores[i]
            else:
                Q[i][j] = Q[i][j+1] * gamma
    ...
    return Variable(torch.Tensor(Q).float().cuda(), requires_grad=False)
