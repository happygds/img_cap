...
from models.critModel import CriticModel
from misc.rewards import init_cider_scorer, get_self_critical_reward, c2f_get_self_critical_reward, get_actor_Q
...
def train(opt):
    ...
    critmodel = models.setup(opt, crictmodel=True)
    critmodel.cuda()
    model.cuda()
    ...
    critic_crit = utils.CriticCriterion()
    # add a new variable to enable actor-critic
    critic_flag = opt.critic

    optimizer = optim.Adam(model.parameters(), lr=opt.learning_rate, weight_decay=opt.weight_decay)
    cricoptimizer = optim.Adam(critmodel.parameters(), lr=opt.learning_rate, weight_decay=opt.weight_decay)

    # Load the optimizer
    # if vars(opt).get('start_from', None) is not None and os.path.isfile(os.path.join(opt.start_from, "optimizer.pth")):
    #     optimizer.load_state_dict(torch.load(os.path.join(opt.start_from, 'optimizer.pth')), strict=False)
    #     cricoptimizer.load_state_dict(torch.load(os.path.join(opt.start_from, 'optimizer.pth')), strict=False)
    ...
    while True:
        ...
        Q_gamma = 1.
        if not sc_flag and not critic_flag:
            loss = crit(model(fc_feats, att_feats, labels), labels[:, 1:], masks[:, 1:])
        elif sc_flag and not critic_flag:
            if opt.caption_model == 'c2ftopdown':
                gen_result, sample_logprobs, gen_result_fine, sample_logprobs_fine, sample_hid, sample_hid_fine = model.sample(
                    fc_feats, att_feats, {'sample_max': 0, 'temperature': opt.temperature})
                Q = get_actor_Q(data, gen_result, Q_gamma, only_cider=True)
                crit_loss, reward = critic_crit(critmodel, sample_hid, gen_result, Q)
                loss = rl_crit(sample_logprobs, gen_result, Variable(
                    torch.from_numpy(reward).float().cuda(), requires_grad=False))
                Q_fine = get_actor_Q(data, gen_result_fine, Q_gamma, only_cider=True)
                loss_fine, reward_fine = critic_crit(critmodel, sample_hid_fine, gen_result_fine, Q_fine)
                ...
                gen_result, sample_logprobs, gen_result_fine, sample_logprobs_fine, sample_hid, sample_hid_fine = model.sample(
                    fc_feats, att_feats, {'sample_max': 0, 'temperature': opt.temperature})
                Q = get_actor_Q(data, gen_result, Q_gamma, only_cider=True)
                crit_loss, _ = critic_crit(critmodel, sample_hid, gen_result, Q)
                Q_fine = get_actor_Q(data, gen_result_fine, Q_gamma, only_cider=True)
                crit_loss_fine, _ = critic_crit(critmodel, sample_hid_fine, gen_result_fine, Q_fine)
                crit_loss += crit_loss_fine
                cricoptimizer.zero_grad()
                crit_loss.backward()
                utils.clip_gradient(cricoptimizer, opt.grad_clip)
                cricoptimizer.step()
            else:
                gen_result, sample_logprobs, sample_hid = model.sample(
                    fc_feats, att_feats, {'sample_max': 0, 'temperature': opt.temperature})
                Q = get_actor_Q(data, gen_result, Q_gamma, only_cider=True)
                crit_loss, reward = critic_crit(critmodel, sample_hid, gen_result, Q)
                loss = rl_crit(sample_logprobs, gen_result, Variable(
            ...
            # print('actor-critic step 2, only trains the critic network')
            if opt.caption_model == 'c2ftopdown':
                gen_result, sample_logprobs, gen_result_fine, sample_logprobs_fine, sample_hid, sample_hid_fine = model.sample(
                    fc_feats, att_feats, {'sample_max': 0, 'temperature': opt.temperature})
                Q = get_actor_Q(data, gen_result, Q_gamma, only_cider=True)
                loss, _ = critic_crit(critmodel, sample_hid, gen_result, Q)
                Q_fine = get_actor_Q(data, gen_result_fine, Q_gamma, only_cider=True)
                loss_fine, _ = critic_crit(critmodel, sample_hid_fine, gen_result_fine, Q_fine)
                loss += loss_fine
        ...
        if iteration % 5 == 0:
            if critic_flag:
                print("iter {} (epoch {}), train_loss = {:.3f}, time/batch = {:.3f}, stage 2, only critic"
        ...
        if (iteration % opt.save_checkpoint_every == 0):
            ...
            if True:  # if true
                ...
                tmpdict = dict(model.state_dict())
                print('model state_dict:', len(tmpdict))
                tmpdict.update(critmodel.state_dict())
                print('complete model state_dict:', len(tmpdict))
                torch.save(tmpdict, checkpoint_path)
