    ...
    def sample(self, fc_feats, att_feats, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            if type(self) == C2FTopDownModel:
                ...
                sample_hid.append(state[0][-1])
                sample_hid_fine.append(state[0][1])
                output, state = self.core(xt, xt_fine, fc_feats, att_feats, p_att_feats, p_att_feats_final, state)
                logprobs = F.log_softmax(self.logit(output[0]))
                logprobsfinal = F.log_softmax(self.logit_final(output[1]))
        ...
        if type(self) == C2FTopDownModel:
            return torch.cat([_.unsqueeze(1) for _ in seq], 1), torch.cat([_.unsqueeze(1) for _ in seqLogprobs], 1), \
                   torch.cat([_.unsqueeze(1) for _ in seq_fine], 1), torch.cat([_.unsqueeze(1) for _ in seqLogprobs_fine], 1), \
                   torch.cat([_.unsqueeze(1) for _ in sample_hid[:-1]], 1), torch.cat([_.unsqueeze(1) for _ in sample_hid_fine[:-1]], 1)
        else:
            return torch.cat([_.unsqueeze(1) for _ in seq], 1), torch.cat([_.unsqueeze(1) for _ in seqLogprobs], 1)
