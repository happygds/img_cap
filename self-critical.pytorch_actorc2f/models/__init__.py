...
from .critModel import *


def setup(opt, crictmodel=False):
    if not crictmodel:
    ...
    if vars(opt).get('start_from', None) is not None:
        ...
        model.load_state_dict(torch.load(os.path.join(opt.start_from, 'model-{}.pth'.format(opt.id))), strict=False)
