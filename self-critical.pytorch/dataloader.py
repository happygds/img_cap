    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            ...
            if ncap < seq_per_img:
                ...
                tmp = self.h5_label_file['labels'][ixl: ixl + seq_per_img, ]

                # process the too long sentences
                for q in range(seq_per_img):
                    q_len = self.label_length[ixl + q] - self.seq_length
                    if q_len > 0:
                        ix_len = random.randint(0, q_len)
                    else:
                        ix_len = 0
                    seq[q, :] = tmp[q, ix_len:ix_len + self.seq_length]
                # gts.append(self.h5_label_file['labels'][ixl: ixl + seq_per_img])
